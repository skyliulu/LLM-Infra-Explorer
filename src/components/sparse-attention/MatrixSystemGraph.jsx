...
function Matrix({ rows, kind, traceId, compact, t, onSelect }) {
  ...
  return <div className={`sm-matrix sm-${kind}`} data-matrix={kind} data-total-rows={rows.length}>
    ...
    {!rows.length && <p className="sm-empty">{t(kind==='reads'?'runUnread':'matrixEmpty')}</p>}
...
function MatrixRow({ row, columns, kind, selected, t, onSelect }) {
  ...
  return <Tag className={`sm-row ${row.read ? 'sm-read' : ''} ${row.chosen?'se-chosen':''} ${row.transferring?'se-transferring':''}`} style={{ '--columns': columns.length }}
    data-matrix-record={row.id} data-read={row.read ?? undefined}
    {...(onSelect ? { onClick: () => onSelect(row), 'aria-pressed': selected,
      'aria-label': `${row.id} · ${t(row.transferring?'runMoving':row.read?'matrixRead':row.chosen?'runChosen':'matrixResident')}` } : {})}>
    <span className="sm-row-id">{row.symbol ? <MathFormula>{row.symbol}</MathFormula> : row.id}</span>
    <MatrixCells values={row.cells} kind={kind}/>
    <span className="sm-read-mark" aria-hidden="true">{row.read ? '✓' : row.chosen ? '○' : ''}</span>
  </Tag>;
}
...
function MatrixCells({ values, kind }) {
  return values.map((value, i) => {
    ...
    return <span key={i} className={`sm-cell ${value < 0 ? 'sm-negative' : 'sm-positive'}`} title={value===null?'—':value.toFixed(5)}
      style={{ '--strength': .08 + Math.min(Math.abs(value) / (weight ? 1 : 1.5), 1) * .26 }}>
      {value===null?'—':weight ? `${(value * 100).toFixed(1)}%` : value.toFixed(2)}
    </span>;
  });
}
...
function GlobalRecords({ m, t, chooseRecord, onFocus, execution:e }) {
  return <div className="sm-global-system">
    {m.indexed && <>
      <section className="sm-index-block sm-block" data-matrix-node="index">
        ...
        {e?.enabled&&<div className="se-index-scores"><small>{t('runScores')}</small>{sampleMatrixRows(m.matrices.index,m.traceId,6).map(r=><span key={r.id}>{r.gap?`⋮ ${r.gap} ${t('matrixOmitted')}`:<>{r.id}: {e.indexReady?r.score.toFixed(2):'—'}</>}</span>)}</div>}
      </section>
...
export function MatrixSystemGraph({ model: m, t, onFocus, chooseRecord, compact, execution:e, playing, projection, onProjection }) {
  ...
  return <ScaledGraph compact={compact} t={t} onFocus={onFocus}><div className={`sm-graph sr-graph sr-fixed ${e?.enabled&&!e.queryReady?'se-query-pending':''}`} data-exec-phase={e?.enabled?e.phase:'overview'} data-playing={playing} data-active-global={e?.activeGlobal} data-active-local={e?.activeLocal} aria-label={t('overview')}>
    ...
    <div className={`sr-branches ${m.hasWindow ? '' : 'sr-single'}`}>
      <div className="sr-branch"><div className="sr-flow-input"><span>{t('flowGlobalIn')}</span><ArrowDown size={18}/></div><GlobalRecords m={m} execution={e} t={t} compact={compact} chooseRecord={chooseRecord} onFocus={onFocus}/>{output('global',m.globalReads,t(m.indexed ? 'matrixSelect' : 'alignedNoIndex'))}</div>
      {m.hasWindow && <div className="sr-branch"><div className="sr-flow-input"><span>{t('flowLocalIn')}</span><ArrowDown size={18}/></div><section className="sr-local sm-block" data-matrix-node="local"><BlockHit id="local" m={m} t={t} onFocus={onFocus}/><div className="sr-branch-caption"><strong>{t('alignedLocal')}</strong><span>{t('readNow')} {e?.enabled?e.localReads:m.local.length}</span></div>{title('local',`${m.local.length}\\times2`)}<Matrix rows={m.matrices.local} kind="local" traceId={m.matrices.traceId} compact={compact} t={t} onSelect={row=>chooseRecord(row,'local')}/><div className="sr-global-caption"><MathFormula>{`L=\\min(N,W)=${m.local.length}`}</MathFormula></div></section>{output('local',e?.enabled?e.localReads:m.local.length,t('alignedAllLocal'))}</div>}
    </div>
    ...
    <section className="sr-attention sm-block" data-matrix-node="attention"><BlockHit id="attention" m={m} t={t} onFocus={onFocus}/>
      <div className="sm-read-set" aria-label={t('jointReadSet')}><span>{t('globalLabel')}</span><b>{m.matrices.selection.globalIds.join(' · ') || t(e?.enabled&&!e.selectionReady?'runUnselected':'matrixEmpty')}</b>{m.hasWindow && <><span>＋ {t('localLabel')}</span><b>{m.matrices.selection.localRange.join('–')}</b></>}<ArrowRight size={13}/><strong>{m.mainReads} {t('readRows')}</strong></div>
