...
import { SystemGraph, NodeMicroscope } from './sparse-attention/AttentionCanvas';
...
export default function SparseAttention() {
  ...
  const t = key => explorerI18n[lang][key] ?? canvasI18n[lang][key] ?? i18n[lang][key] ?? key;
  const pathRef = useRef(null);
  const update = patch => setInput(old => ({ ...old, ...patch }));
  const onFocus = value => {
  ...
  return <div className="sparse-module sc-module bg-slate-50 text-slate-800" data-testid="sparse-module" data-mode={model.mode} data-focus={model.focus} lang={lang}>
    <header className="sa-card sa-header">
      ...
      <div className="sa-header-controls"><div className="sa-segment" role="group" aria-label={t('strategy')}>{['dsa','csa','hca'].map(mode => <button key={mode} aria-pressed={model.mode === mode} onClick={() => { update({ mode }); setFocus('overview'); }}>{t(`strategy_${mode}`)}</button>)}</div><div className="sa-actions"><button className="sa-language" onClick={() => setLang(lang === 'zh' ? 'en' : 'zh')} aria-label={t('language')}><Globe size={16}/>{lang === 'zh' ? 'EN' : '中文'}</button><button className="sa-icon" aria-label={t('reset')} title={t('reset')} onClick={() => { setInput(CANVAS_DEFAULTS); setFocus('overview'); }}><RotateCcw size={18}/></button></div></div>
    </header>
    <section className="sc-canvas" aria-label={t('overview')} data-testid="attention-canvas">
      <div className="sc-experiment"><div><strong>{t('why')}</strong><small>{t('experiment')}</small></div><label className="sc-length"><span>{t('tokens')} <b>{model.tokens}</b></span><input aria-label={t('tokens')} type="range" min="1" max="64" value={model.tokens} onChange={e => update({tokens:+e.target.value})}/></label><div className="sa-segment" role="group" aria-label={t('query')}>{[0,1].map(q => <button key={q} aria-pressed={model.query === q} onClick={() => update({query:q})}>{t(q ? 'queryB' : 'queryA')}</button>)}</div></div>
      <ResourceComparison model={model} t={t} update={update} onFocus={onFocus}/>
      <div className="sc-path" ref={pathRef} tabIndex={-1}><nav aria-label={t('drillPath')}><button onClick={() => onFocus('overview')} disabled={model.focus === 'overview'}>{model.focus !== 'overview' && <ArrowLeft size={14}/>} {t('overview')}</button>{model.focus !== 'overview' && <><span>/</span><strong>{t(`${model.focus}Node`)}</strong>{model.tracedRecord && <><span>/</span><span>{model.tracedRecord.id}</span></>}</>}</nav><small>{t('zoomHint')}</small></div>
      <div className={`sc-workspace ${model.focus !== 'overview' ? 'sc-zoomed' : ''}`} data-testid="canvas-workspace">
        <SystemGraph model={model} t={t} onFocus={onFocus} compact={model.focus !== 'overview'}/>
        {model.focus !== 'overview' && <section className="sc-microscope" aria-label={t(`focus_${model.focus}`)} key={model.focus}>
          ...
          <div className="sc-detail-scroll" tabIndex={0}><LocalImpact m={model} t={t}/><RecordTrace m={model} t={t} onFocus={onFocus}/><p className="sc-detail-desc">{t(`desc_${model.focus}`)}</p><NodeMicroscope model={model} t={t} update={update} chooseRecord={chooseRecord} onFocus={onFocus}/></div>
        </section>}
      </div>
