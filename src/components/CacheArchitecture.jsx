...
export default function CacheArchitecture(){
 const [lang,setLang]=useState(()=>navigator.language.toLowerCase().startsWith('zh')?'zh':'en');
 const [tokens,setTokens]=useState(4096),[layer,setLayer]=useState(20),[phase,setPhase]=useState('prefill'),[entry,setEntry]=useState(0);
 ...
 return <div className="ca-page"><header className="ca-top module-header-card"><div><h1>{t('title')}</h1><p>{t('subtitle')}</p></div><button className="ca-language" aria-label={t('language')} onClick={()=>setLang(v=>v==='zh'?'en':'zh')}><Globe size={16}/>{t('toggle')}</button></header>
 <main><BusinessOverview m={m} lang={lang}><div className="ca-business-controls"><label>{t('tokens')}<select aria-label={t('tokens')} value={tokens} onChange={e=>{setTokens(Number(e.target.value));setEntry(0);}}>{[1,32,129,4096,1000000].map(n=><option key={n} value={n}>{number(n)}</option>)}</select></label><div className="ca-toggle" role="group" aria-label={t('phase')}>{['prefill','decode'].map(v=><button key={v} aria-pressed={phase===v} onClick={()=>setPhase(v)}>{t(v)}</button>)}</div></div></BusinessOverview> <TechnicalFlow key={`${tokens}-${phase}`} m={m} lang={lang} ShapeTensor={ShapeTensor} t={t}/><section className="ca-budget"><div className="ca-heading"><div><h2>{t('budget')}</h2><p>{t('budgetHint')}</p></div></div>
 ...
 <section className="ca-workbench"><div className="ca-heading"><div><h2>{t('overview')}</h2><p>{t('inspect')}</p></div></div>
...
<details className="ca-evidence"><summary>{t('sourceTitle')}</summary><p>{t('sourceNote')}</p>{[['config','inference/config.json'],['implementation','inference/model.py'],['report','DeepSeek_V41_Tech_Report.pdf']].map(([key,path])=><a key={key} href={sourceBase+path} target="_blank" rel="noreferrer">{t(key)} ↗</a>)}</details></main></div>;
}
