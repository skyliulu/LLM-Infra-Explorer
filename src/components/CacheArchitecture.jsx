...
import SessionLifecycle from './cache-architecture/SessionLifecycle';
...
export default function CacheArchitecture(){
 ...
 return <div className="chapter-page ca-page"><header className="ca-top module-header-card chapter-header"><div><h1><ChapterIcon chapter="cachearch"/>{t('title')}</h1><p>{t('subtitle')}</p><div className="ca-mechanism-map">{['cedRole','sharingRole','precisionRole','indexRole'].map(key=><span key={key}>{t(key)}</span>)}</div></div></header>
 <div className="chapter-body"><BusinessOverview m={deriveCacheArchitectureModel({tokens,phase:'prefill'})} lang={lang}><div className="ca-business-controls"><label>{t('tokens')}<select aria-label={t('tokens')} value={tokens} onChange={e=>{setTokens(Number(e.target.value));setEntry(0);}}>{[1,32,129,4096,1000000].map(n=><option key={n} value={n}>{number(n)}</option>)}</select></label></div></BusinessOverview> <TechnicalFlow key={`${tokens}-${phase}`} m={m} lang={lang} ShapeTensor={ShapeTensor} t={t} controls={<div className="ca-toggle" role="group" aria-label={t('phase')}>{['prefill','decode'].map(v=><button key={v} aria-pressed={phase===v} onClick={()=>setPhase(v)}>{t(v)}</button>)}</div>}/><section className="ca-workbench"><div className="ca-heading"><div><h2 data-section-anchor="cachearchitecture-2">{t('overview')}</h2><p>{t('inspect')}</p></div></div>
...
<section className="ca-budget"><div className="ca-heading"><div><h2 data-section-anchor="cachearchitecture-1">{t('budget')}</h2><p>{t('budgetHint')}</p></div></div>
 ...
 <section className="ca-index-section"><HierarchicalIndexer key={`${tokens}-${layer}-${phase}`} m={m} lang={lang} onSelect={select}/></section>
<SessionLifecycle key={tokens} tokens={tokens} lang={lang}/>
