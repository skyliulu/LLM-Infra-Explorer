...
后续执行进度（2026-09-11）：第 1 项 KV 表示显微镜已验收并推送 `main`（`f653ad7`）；第 2 项 Sparse Attention 新章节及六项 review 改进完成验收，纳入本次提交，趋势默认展开。第 3–4 项待推进。具体证据见 `design-qa.md` 当日记录，以下研究与优先级保留为规划依据。
