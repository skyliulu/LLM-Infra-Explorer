...
后续执行进度（2026-09-12）：第 1 项 KV 表示显微镜已验收并推送 `main`（`f653ad7`）；第 2 项 Sparse Attention 已推送（`2ce0d2f`）。第 3 项 FP4 数值与 KV 存储布局已融入量化章节顶部权重格式与 01 的独立 KV 精度入口，并完成模型回归、浏览器验收和生产构建，待用户查看，尚未提交。第 4 项待推进。具体证据见 `design-qa.md`，以下研究与优先级保留为规划依据。
