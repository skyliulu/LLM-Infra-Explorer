...
后续执行进度（2026-09-12）：第 1 项 KV 表示显微镜已验收并推送 `main`（`f653ad7`）；第 2 项 Sparse Attention 已推送（`2ce0d2f`）。第 3 项 FP4 数值与 KV 存储布局已融入量化章节顶部权重格式与 01 的独立 KV 精度入口，并完成模型回归、浏览器验收和生产构建，已提交推送（`db1708a`）。第 4 项的 40 层共享拓扑、记录账本和 CED 路径首版已实现并验收，尚未提交；分层候选池的数值打分与持久化缓存/Encoder 回放作为后续扩展。具体证据见 `design-qa.md`，以下研究与优先级保留为规划依据。
