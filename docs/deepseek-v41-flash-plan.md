...
后续执行进度（2026-09-12）：第 1 项 KV 表示显微镜已验收并推送 `main`（`f653ad7`）；第 2 项 Sparse Attention 已推送（`2ce0d2f`）。第 3 项 FP4 数值与 KV 存储布局已融入量化章节顶部权重格式与 01 的独立 KV 精度入口，并完成模型回归、浏览器验收和生产构建，已提交推送（`db1708a`）。第 4 项的 40 层共享拓扑、记录账本和 CED 路径首版与动态执行已验收并推送（`f6fab5c`）。分层候选池的分块筛选、数值重选与地址读取已在本地实现；连续 Token 缓存生命周期、持久化缓存/Encoder 回放仍作为后续扩展。具体证据见 `design-qa.md`，以下研究与优先级保留为规划依据。

第 2 项最新交互修订：默认显示全量 Attention 与当前机制的缓存、每步读取量、预算可容纳历史对照，三项趋势同时展开；在同一画布内放大协作分支、记录和通道。收益分项可定位部件，记录可在缓存/索引/输出之间双向追踪，汇合视图包含信息覆盖和二维输出参照。六项 review 后通过96组代表节点状态、32组边界状态以及资源/取舍模型回归；详细模型边界见设计与QA记录。

后续扩展（2026-09-13，本地待 review）：连续 Token 缓存生命周期和多轮会话恢复现已接入同一 CED 会话画布。包含配对压缩暂存、逐层 SWA 窗口、短 TTL 检查点、全局记录回载、Encoder 有限回放、Decoder 尾部状态构建与继续生成。具体模型、指标口径和运行时边界见 `docs/ced-session-design.md`；不新增章节，不重复 HiCache 的介质调度。
...
| Hierarchical Sparse Indexer | decoder 后续 Reindex 只允许候选池内位置参与选择；参考代码先全量打分再掩码 | 首个 Full 仍扫描全部因果可见位置；不能宣称全部索引成本与上下文无关 |
